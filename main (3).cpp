...
void levelUp(player& p)
{
    ...
    {
        p.level++;
        cout << "You've leveled up! Your level is now " << p.level << "." << endl;
    }
    else
    {
        int howMuchExp = expcap - p.expp;
        cout << "You need " << howMuchExp << " exp more to level up." << endl;
    }
}
...
clearcons();
    ...
    else if (p.HP > 0 && e.HP <= 0)
    {
        ...
        cout << "You've gained " << exppGain << " exp! You now have " << p.expp << " exp." << endl;
        expCap(p);
        levelUp(p);
    }
}
           ...
           if (!weapons.empty() && !enemies.empty())
            {
                ...
                while(!validEnemy)
                {
                    randomIndex = rand() % enemies.size(); // chooses random enemy
                    selectedEnemy = enemies[randomIndex];
                    
                    if (selectedEnemy.level <= p.level)
                    {
                        validEnemy = true;
                    }
                }

                cout << "Going further, you've noticed " << selectedEnemy.name << " on your way!" << endl;
                ...
                fight(p, selectedWeapon, selectedEnemy);
            }
            else
            {
                exit(0);
